...
Open `/debugging` locally (or choose **Errors & stacks**) to import a Python, JavaScript or .NET stack, or Sentry REST issue-event JSON. The local backend bounds and sanitizes input; the page displays ordered frames, exception chains, breadcrumbs, omissions and a reviewed JSON download. SDK ingestion payloads are deliberately unsupported. Choose **Save investigation** to retain sanitized evidence independently of analysis jobs. Cases are stored locally in `backend/.investigations` by default (`CODANDY_INVESTIGATION_ROOT`), up to 100 cases of 4 MiB each. You can reopen a case, edit scrubbed notes/title/status, and explicitly delete it. Unsaved imports are held in a bounded 20-observation backend cache; raw payloads are not persisted.
...
The hosted page describes this local workflow; it does not forward telemetry to a hosted API. Saved investigations can match stack paths against a retained repository snapshot, inspect captured source, and prepare a reviewed/cited debugging brief for export or local Codex Q&A. Matches remain candidates; supplied commit IDs are not independent verification, and source snapshots may expire while case evidence remains. AI requests reject stale review digests and citations outside the packet. Project/issue browsing remains subsequent D1 work. Live access requires user configuration and has not been verified against a real account.


## ZIP uploads and Architecture maps

...
export const ticketReviewSchema = z.object({ schema_version: z.literal("ticket-review-0.1"), provider: z.literal("linear"), payload: z.object({ teamId: z.string().uuid(), title: z.string(), description: z.string() }), digest: z.string().regex(/^[a-f0-9]{64}$/), redactions: z.number().int().nonnegative(), previous_submissions: z.number().int().nonnegative().default(0) });

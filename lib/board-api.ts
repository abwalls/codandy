...
const finding = z.object({ text: z.string(), element_ids: z.array(z.string()), basis: z.enum(["drawn", "answered", "assumed", "visual_inference"]) });
...
export const boardSchema = z.object({ schema_version: z.literal("board-0.1"), id: z.string().uuid(), revision: z.number().int(), title: z.string(), notes: z.string(), snapshot_id: z.string().nullable(), scene: z.object({ elements: z.array(z.record(z.unknown())) }), artifacts: z.array(z.object({ id: z.string(), revision: z.number(), stage: z.enum(["interpret", "plan"]), model: z.string(), visual_input: z.boolean().default(false), interpretation: interpretation.nullable(), plan: z.object({ title: z.string(), objective: z.string() }).passthrough().nullable() })) });

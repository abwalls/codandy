...
export function CreateTicket({ seed }: { seed: TicketSeed }) {
  ...
  function invalidate() { setReview(null); setChecked(false); setReceipt(null); setSubmission(""); }
  ...
  if (!open) return <Button variant="outline" onClick={() => setOpen(true)}>Prepare Linear ticket</Button>;
  return <section aria-label="Prepare Linear ticket" className="space-y-3 rounded-xl border bg-card p-4 text-foreground">
    ...
    <Button disabled={busy || !team || !title.trim() || title.length > 250 || description.length > 16000} onClick={() => void action(async () => { setReview(ticketReviewSchema.parse(await integrationRequest("/tickets/review", draft))); setSubmission(crypto.randomUUID()); setChecked(false); setReceipt(null); })}>Review exact payload</Button>
    {review && <><p className="text-xs text-muted-foreground">Best-effort secret scrubbing applied. Check the entire text and team ID before sharing.</p><pre className="max-h-80 overflow-auto whitespace-pre-wrap break-all rounded border p-3 text-xs">{JSON.stringify(review.payload, null, 2)}</pre>
      ...
      <Button disabled={busy || !checked || !!receipt} onClick={() => void action(async () => { setReceipt(ticketReceiptSchema.parse(await integrationRequest("/tickets", { ...draft, digest: review.digest, idempotency_key: submission, confirmed: true }))); })}>{busy ? "Working…" : "Create ticket in Linear"}</Button></>}

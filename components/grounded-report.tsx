...
function EvidenceCard({ item, nodes, onInspect }: { item: GroundedReportItem; nodes: Map<string, AnalysisAtlas["nodes"][number]>; onInspect: (id: string) => void }) {
...
export function GroundedSection({ section, atlas, onInspect, recommendations = false }: { section: GroundedReportSection; atlas: AnalysisAtlas; onInspect: (id: string) => void; recommendations?: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    {section.items.length > 0 ? <>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-2">{filtered.slice(0, limit).map(item => <EvidenceCard key={item.id} item={item} nodes={nodes} onInspect={onInspect} />)}</div>

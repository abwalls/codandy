...
function LiveReportContent({ atlas, jobId, onExit }: { atlas: AnalysisAtlas; jobId: string | null; onExit: () => void }) {
  ...
  return <main className="atlas-grid min-h-svh bg-[var(--background)] text-slate-100">
    ...
    <div className="mx-auto grid max-w-[1600px] gap-6 p-5 lg:grid-cols-[210px_minmax(0,1fr)]">
      <nav aria-label="Report sections" className="flex flex-wrap content-start gap-2 lg:flex-col"><Button variant="ghost" className="justify-start" asChild><Link href="/debugging">Errors & stacks</Link></Button><Button variant="outline" asChild><Link href="/whiteboard">Whiteboard</Link></Button>{sections.map(item => <Button key={item} variant="ghost" aria-current={section === item ? "page" : undefined} className={`justify-start ${section === item ? "bg-cyan-300/10 text-cyan-200" : "text-slate-400"}`} onClick={() => setSection(item)}>{item}</Button>)}<p className="p-3 text-xs text-slate-500">Ask Codandy can use your local ChatGPT/Codex connection. Your plan’s usage limits apply.</p></nav>

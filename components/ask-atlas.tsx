...
export function useAskAtlas() { return useContext(AskContext); }
export function AskButton({ scope, children = "Ask Atlas" }: { scope: AskScope; children?: ReactNode }) {
...
function QuestionDraft({ scope, context, jobId }: { scope: AskScope; context: unknown; jobId?: string | null }) {
  ...
  function download() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = "atlas-question.txt"; link.click();
  ...
  return <div className="space-y-4 px-4 pb-6">
    ...
    <label className="block text-sm">Your question<textarea autoFocus aria-label="Your Atlas question" maxLength={4000} rows={4} className="mt-2 block w-full rounded-lg border border-white/15 bg-card p-3" value={question} onChange={event => { setQuestion(event.target.value); setStatus(""); }} /></label>
    ...
    <p className="text-xs text-slate-400">Only the displayed excerpt is included. Review it before sharing. Answers from another assistant are not verified Code Atlas findings.</p>
  </div>;
}

export function AskAtlasProvider({ children, contextFor, jobId }: { children: ReactNode; contextFor: (scope: AskScope) => unknown; jobId?: string | null }) {
  ...
  return <AskContext.Provider value={scope => setRequest({ scope, context: contextFor(scope) })}>{children}
    <Sheet open={!!request} onOpenChange={open => { if (!open) setRequest(null); }}><SheetContent className="w-[min(600px,95vw)] overflow-y-auto bg-background text-foreground sm:max-w-[600px]">
      <SheetHeader><SheetTitle>Ask Atlas</SheetTitle><SheetDescription>Draft a question scoped to this page or selected evidence.</SheetDescription></SheetHeader>

...
## 2026-09-16 browsing and source-link checkpoint

Source-specific receipts and additional-ticket acknowledgement are implemented. The ledger remains body/credential-free. Recommendations use repository/revision scope and board links use stable board IDs; reusing an exact completed submission still returns its receipt. Local link lookup never contacts Linear.

Sentry projects and issues now share the existing bounded event transport. Projects use per_page=50 and require org:read; issues use limit=50, a selected numeric project and shortIdLookup=0. Cursor metadata is validated and URLs in Link headers are never followed. Malformed, oversized or foreign-project responses fail closed. Browser lists retain only scrubbed display fields in tab state. No automatic provider call occurs from opening the browser controls, and selecting an issue only fills its ID.

Live account verification, release commits, shared cross-provider registry/transport and read retry-after handling remain open. Reconfirmed primary references: https://docs.sentry.io/api/organizations/list-an-organizations-projects/, https://docs.sentry.io/api/events/list-an-organizations-issues/ and https://docs.sentry.io/api/pagination/.
